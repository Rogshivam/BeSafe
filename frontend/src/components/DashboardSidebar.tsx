...
const sidebarItems = [
  ...
  { title: 'Threat Logs', icon: FileWarning, url: '' },
  { title: 'Incident Reports', icon: FileText, url: '' },
  { title: 'Emergency Contacts', icon: Users, url: '' },
...
export const DashboardSidebar = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static top-0 left-0 z-50 h-screen bg-dash-dark text-primary-foreground transition-all duration-300 flex flex-col
        ${collapsed ? 'w-20' : 'w-64'}
      ...
      `}>
        <div className="p-4 flex items-center justify-between border-b border-dash-surface">
          ...
          <button className="hidden lg:block opacity-60 hover:opacity-100" onClick={() => setCollapsed(!collapsed)}>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors
                  ${isActive ? 'bg-primary text-primary-foreground' : 'text-primary-foreground/70 hover:bg-dash-surface hover:text-primary-foreground'}
                `}
              >
        ...
        <div className="p-3 border-t border-dash-surface">
          <button
            onClick={logout}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-primary-foreground/70 hover:bg-dash-surface transition-colors ${collapsed ? 'justify-center' : ''}`}
          >
            <X className="w-5 h-5" />
