import { Link, useLocation } from 'react-router-dom';
import { ShieldCheck, LayoutDashboard, FolderLock, FileText, Users, Settings, X, Menu, UserPlus, Users2 } from 'lucide-react';
import { useState } from 'react';
import { useAuth } from '@/context/AuthContext';

const baseSidebarItems = [
  { title: 'Dashboard', icon: LayoutDashboard, url: '' },          // will expand to `/dashboard/[role]`
  { title: 'Evidence Locker', icon: FolderLock, url: '/evidence-locker' },
  { title: 'Incident Reports', icon: FileText, url: '/incidents' },
  { title: 'Emergency Contacts', icon: Users, url: '/emergency-contacts' },
  { title: 'Settings', icon: Settings, url: '/settings' },
];

const parentSidebarItems = [
  ...baseSidebarItems,
  { title: 'Child Info', icon: Users2, url: '/child-info' },
];

const childAdultSidebarItems = [
  ...baseSidebarItems,
  { title: 'Parent Info', icon: UserPlus, url: '/parent-info' },
];

export const DashboardSidebar = () => {
  const [collapsed, setCollapsed] = useState(false);
  const [mobileOpen, setMobileOpen] = useState(false);
  const { role, userName, logout } = useAuth();
  const location = useLocation();

  // Get role-specific sidebar items
  const getSidebarItems = () => {
    switch (role) {
      case 'parent':
        return parentSidebarItems;
      case 'child':
      case 'adult':
      case 'individual':
        return childAdultSidebarItems;
      default:
        return baseSidebarItems;
    }
  };

  const sidebarItems = getSidebarItems();

  const getUrl = (item: typeof sidebarItems[0]) => {
    if (item.title === "Dashboard") {
      return `/dashboard/${role || 'adult'}`;
    }
    return item.url;
  };

  return (
    <>
      {/* Mobile Hamburger Trigger (Only on screens < lg) */}
      <button
        type="button"
        aria-label="Open Navigation Menu"
        className="lg:hidden fixed top-4 left-4 z-40 w-10 h-10 gradient-primary text-primary-foreground rounded-xl flex items-center justify-center shadow-lg hover:scale-105 active:scale-95 transition-all"
        onClick={() => setMobileOpen(true)}
      >
        <Menu className="w-5 h-5" />
      </button>

      {/* Mobile Drawer Overlay */}
      {mobileOpen && (
        <div 
          className="lg:hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-50 transition-opacity" 
          onClick={() => setMobileOpen(false)} 
        />
      )}

      {/* Sidebar: Sticky Desktop, Drawer on Mobile */}
      <aside
        className={`fixed lg:sticky top-0 left-0 z-50 h-screen bg-dash-dark text-primary-foreground transition-all duration-300 flex flex-col shrink-0
          ${collapsed ? 'w-20' : 'w-64'}
          ${mobileOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full lg:translate-x-0'}`}
      >
        <div className="p-4 flex items-center justify-between border-b border-dash-surface shrink-0">
          <Link to="/" className="flex items-center gap-2">
            <div className="w-8 h-8 gradient-primary rounded-lg flex items-center justify-center">
              <ShieldCheck className="w-5 h-5" />
            </div>
            {!collapsed && <span className="font-bold text-lg">Be-Safe</span>}
          </Link>
          <button
            type="button"
            className="hidden lg:block opacity-60 hover:opacity-100 p-1"
            onClick={() => setCollapsed(!collapsed)}
          >
            {collapsed ? <Menu className="w-4 h-4" /> : <X className="w-4 h-4" />}
          </button>
          <button 
            type="button"
            className="lg:hidden opacity-80 hover:opacity-100 p-1" 
            onClick={() => setMobileOpen(false)}
          >
            <X className="w-5 h-5" />
          </button>
        </div>

        {!collapsed && (
          <div className="px-4 py-3 border-b border-dash-surface shrink-0">
            <p className="text-xs opacity-60">Welcome back,</p>
            <p className="font-semibold text-sm text-primary truncate">{userName || 'Member'}!</p>
          </div>
        )}

        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {sidebarItems.map((item) => {
            const currentUrl = getUrl(item);
            const isActive = 
              location.pathname === currentUrl || 
              (item.title === 'Dashboard' && location.pathname.startsWith('/dashboard'));

            return (
              <Link
                key={item.title}
                to={currentUrl}
                onClick={() => setMobileOpen(false)}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
                  ${isActive
                    ? 'gradient-primary text-primary-foreground shadow-sm font-semibold'
                    : 'text-primary-foreground/70 hover:bg-dash-surface hover:text-primary-foreground'}`}
              >
                <item.icon className="w-5 h-5 flex-shrink-0" />
                {!collapsed && <span>{item.title}</span>}
              </Link>
            );
          })}
        </nav>

        <div className="p-3 border-t border-dash-surface shrink-0">
          <button
            type="button"
            onClick={logout}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-primary-foreground/70 hover:bg-dash-surface transition-colors ${
              collapsed ? 'justify-center' : ''
            }`}
          >
            <X className="w-5 h-5" />
            {!collapsed && <span>Logout</span>}
          </button>
        </div>
      </aside>
    </>
  );
};